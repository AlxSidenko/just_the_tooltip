...
abstract class JustTheTooltipState<T> extends State<JustTheInterface>
  ...
  Future<void> _hideTooltip({bool immediately = true}) async {
    cancelShowTimer();
      ...
      if (_mouseIsConnected) {
        result = MouseRegion(
          onEnter: (PointerEnterEvent event) {
            Future.delayed(widget.showDuration!).then((value) {
              if (!isTooltipHovered) {
                _showTooltip();
              }
            });
  ...
  Widget _createSkrim() {
    if (barrierBuilder != null) {
      return Container(
        key: skrimKey,
        child: barrierBuilder!(
            context, _animationController, () {} /*_hideTooltip*/),
      );
    }
          ...
          child: Builder(
            builder: (context) {
              ...
              final wrappedChild = Material(
                ...
                child: MouseRegion(
                    onEnter: (event) => isTooltipHovered = true,
                    onHover: (event) => isTooltipHovered = true,
                    onExit: (event) {
                      isTooltipHovered = false;
                      Future.delayed(widget.showDuration!).then((value) {
                        if (!isTooltipHovered) {
  ...
  TargetInformation _getTargetInformation(BuildContext context) {
    ...
    Offset? updatedTarget;
    if (target.dx < 471) {
      updatedTarget = Offset(471, target.dy);
    }
